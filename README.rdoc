...
If you are making changes to the javascript files,
you need to cd into app/javascripts and run bin/compile
This will auto-compile the javascript chagnes you make
...
To install the required plugins, run the following commands:
  ...
  $ script/plugin install git://github.com/mislav/will_paginate.git -r 2-3-stable
  
  $ script/plugin install git://github.com/berk/will_filter.git -r rails-2.3.x
  $ rake will_filter:sync  
  $ rake db:migrate 
  $ ln -s ../vendor/plugins/will_filter/public/wf public/wf

To install, configure and initialize Tr8n engine, run the following commands: 

  $ script/plugin install git://github.com/berk/tr8n.git -r rails-2.3.x
  $ rake tr8n:sync         
  $ rake db:migrate 
  $ rake tr8n:init
  $ ln -s ../vendor/plugins/tr8n/public/tr8n public/tr8n
